...
type Config struct {
	...
	Tangled struct {
		...
		// WebBaseURL is Tangled's own browsable web frontend
		// (https://tangled.cyberwild.org, distinct from PDSBaseURL, the
		// XRPC/AT Proto endpoint) — used to build a trackback link for a
		// comment relayed back to Graft. Optional: empty just omits it.
