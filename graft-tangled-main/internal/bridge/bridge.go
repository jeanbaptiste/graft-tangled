...
type Bridge struct {
	...
	Series    []SeriesConfig
	Opts      Options
	// TangledWebBaseURL and TangledHandle build each relayed comment's
	// own public permalink (https://<host>/<handle>/<repo>/issues/<N>)
	// for Graft's wiki trackback link — the web UI addresses a repo's
	// owner by handle, not DID, unlike everywhere else this bridge deals
	// in DIDs.
	TangledWebBaseURL string
	TangledHandle     string
...
func (b *Bridge) reverseTangledToGraft(ctx context.Context) error {
	...
	for _, c := range comments {
		...
		sourceURL := ""
		if b.TangledWebBaseURL != "" && b.TangledHandle != "" {
			if rkey, issueID, err := b.AppviewDB.IssueLocator(ctx, c.IssueAt); err == nil {
				sourceURL = fmt.Sprintf("%s/%s/%s/issues/%d", strings.TrimRight(b.TangledWebBaseURL, "/"), b.TangledHandle, rkey, issueID)
			}
		}
